...
const FormBuilder = () => {
    ...
    const chosenPage = useCallback((pageId) => {
        ...
        const page = currentForm?.pages.find(p => p.pageId === pageId);
    ...
    const handlePublishOrSave = async () => {
        try {
            ...
            if (response && response.data) {
                ...
                const publishableLink = `http://localhost:5173/striform/form/${savedForm.formId}`;

                const updatedForm = { ...savedForm, publishableLink };
                const updateLinkResponse = await axios.put(`https://striform-backend-1.onrender.com/api/forms/${savedForm.formId}`, updatedForm, config);
