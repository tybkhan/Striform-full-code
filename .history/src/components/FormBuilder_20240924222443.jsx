...
const FormBuilder = () => {
    ...
    const [currentForm, setCurrentForm] = useState(location.state?.formData);
    ...
    const handlePublishOrSave = async () => {
        try {
            ...
            if (currentForm._id) {
                ...
                response = await axios.put(`http://localhost:4000/api/forms/${currentForm._id}`, formData);
            } else {
                console.log("Creating new form");
                response = await axios.post('http://localhost:4000/api/forms/submit', formData, config);
            }
