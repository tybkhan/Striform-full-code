...
const LongText = ({ pageData }) => {
  ...
  const [formState, setFormState] = useState(() => ({
    ...
    componentsMetaData: {
      ...
      textBox: {
        value: pageData?.componentsMetaData?.textBox?.value || "",
        maxVal: pageData?.componentsMetaData?.textBox?.maxVal || defaultMetadata.textBox.maxVal,
        size: pageData?.componentsMetaData?.textBox?.size || defaultMetadata.textBox.size,
      },
  ...
  return (
    <>
      <div id="viewer" className="w-full px-20 space-y-4 pointer-events-none">
        <FrmQuestion text={formState.componentsMetaData.question.val} />
        <FrmTxtBox
