...
const defaultMetadata = availablePages.find(page => page.pageName === "Dropdown List").componentsMetaData;
...
const DropdownLst = ({ pageData }) => {
  ...
  const [formState, setFormState] = useState(() => ({
    ...
    componentsMetaData: {
      ...
      dropdown: {
        options: pageData?.componentsMetaData[0]?.dropdown?.options ?? defaultMetadata.dropdown.options
      },
      button: { 
    ...
    if (isInitialMount.current) {
      ...
      setFormState(prevState => ({
        ...
        componentsMetaData: {
          ...
          dropdown: {
            options: pageData?.componentsMetaData[0]?.dropdown?.options ?? prevState.componentsMetaData.dropdown.options
          },
          button: { 
  ...
  return (
    <>
      <div id="viewer" className="w-full px-20 space-y-4 pointer-events-none">
        ...
        {formState.componentsMetaData[0]?.dropdown?.options.length > 0 && <FrmDrpdwn options={formState.componentsMetaData[0]?.dropdown?.options} />}
      ...
      <div id="controls" className="w-[27vw] p-4 h-[86.9vh] fixed bg-gray-100 bottom-0 -right-10 flex flex-col space-y-4 overflow-y-auto">
        ...
        />
        <FrmDrpdwnController
          options={formState.componentsMetaData[0]?.dropdown?.options}
