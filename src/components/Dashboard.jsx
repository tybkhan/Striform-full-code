...
const Dashboard = () => {
    ...
    const fetchForms = async () => {
        ...
        try {
            ...
            // Remove local forms that have been published
            const updatedLocalForms = localForms.filter(localForm => 
                !serverForms.some(serverForm => serverForm.formId === localForm.formId)
            );

            const allForms = [...serverForms, ...updatedLocalForms];
            setForms(allForms);
    ...
    const handleDelete = async (form, e) => {
        ...
        try {
            if (form.isLocal) {
                ...
                const response = await axios.delete(`https://striform-backend-1.onrender.com/api/forms/${form._id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
    ...
    const handleFormClick = async (formId) => {
        try {
            ...
            let formData;
            if (!form.isLocal) {
                const response = await axios.get(`https://striform-backend-1.onrender.com/api/forms/${form._id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                });
            ...
            updateForm(formData);
            navigate(`/form-builder/${formId}`, { state: { formData } });
        } catch (error) {
